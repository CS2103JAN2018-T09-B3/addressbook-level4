...
:repoURL: https://github.com/se-edu/progresschecker-level4/tree/master
...
Note how components of ProgressChecker have well-defined APIs. For example, the API of the `Logic` component is given in the link:{repoURL}/src/main/java/seedu/address/logic/Logic.java[`Logic.java`]
...
Note how the ProgressChecker app uses Java ``assert``s to verify assumptions.
...
* Add more assertions to ProgressChecker as you see fit.
...
Note <<DeveloperGuide#logging, how the ProgressChecker app uses Java's `java.util.log` package to do logging>>.
...
Add more logging to ProgressChecker as you see fit.
...
Note how ProgressChecker uses the `ReadOnly*` interfaces to prevent objects being modified by clients who are not supposed to modify them.
...
Analyze the ProgressChecker code/design to identify,
...
Note <<UsingGradle#, how the ProgressChecker app uses Gradle to automate build tasks>>.
...
Note <<UsingTravis#, how the ProgressChecker app uses Travis to perform Continuous Integration>>. (https://travis-ci.org/se-edu/progresschecker-level4[image:https://travis-ci.org/se-edu/progresschecker-level4.svg?branch=master[Build Status]])
...
Note how our CI server <<UsingTravis#, Travis uses Coveralls to report code coverage>>. (https://coveralls.io/github/se-edu/progresschecker-level4?branch=master[image:https://coveralls.io/repos/github/se-edu/progresschecker-level4/badge.svg?branch=master[Coverage Status]]) After <<UsingCoveralls#, setting up Coveralls>> for your project, you can visit Coveralls website to find details about the coverage of code pushed to your repo. https://coveralls.io/github/se-edu/progresschecker-level4?branch=master[Here] is an example.
...
* Test methods `prefsReadSave()` and `progressCheckerReadSave()` are integration tests. Note how they simply test if The `StorageManager` class is correctly wired to its dependencies.
* Test method `handleProgressCheckerChangedEvent_exceptionThrown_eventRaised()` is a unit test because it uses _dependency injection_ to isolate the SUT `StorageManager#handleProgressCheckerChangedEvent(...)` from its dependencies.
...
* Note how some PRs in this project have been reviewed by other developers. Here is an https://github.com/se-edu/progresschecker-level4/pull/147[example].
* Also note how we have used https://www.codacy.com[Codacy] to do automate some part of the code review workload (https://www.codacy.com/app/damith/progresschecker-level4?utm_source=github.com&utm_medium=referral&utm_content=se-edu/progresschecker-level4&utm_campaign=Badge_Grade[image:https://api.codacy.com/project/badge/Grade/fc0b7775cf7f4fdeaf08776f3d8e364a[Codacy Badge]])
