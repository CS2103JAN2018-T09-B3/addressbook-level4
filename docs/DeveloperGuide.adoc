...
The LOs and todos for all weeks would be stored as a local file upon which it would be loaded by the `StorageManager` on start up.
...
To use Google Tasks API, we first add corresponding dependencies to build.gradle, the JAR files will be downloaded automatically upon rebuild.

[NOTE]
Simply downloading JAR files without editing gradle is incorrect. JARs are not in git thus our co-developers will rely on the dependencies to retrieve the libraries.


It also help us save time of building massive data structures (ie. Tasks, TaskLists, Lists of TaskLists).
However, we do have a few classes in the modeling part that further add more customized methods which are useful for current commands and even future commands.
